...
// watches application log channels, writes csv rows
public sealed class AppEventLogSource : IDisposable
{
    // default application channels
    public static readonly string[] DefaultChannels =
    [
        "Application",   // primary application event log
    ];
    ...
    public AppEventLogSource(EventCsvWriter writer, string[]? channels = null)
    {
        _writer = writer;
    ...
    private void OnEventRecordWritten(object? sender, EventRecordWrittenEventArgs e)
    {
        ...
        if (!EventCatalog.TryClassifyApplication(record.Level, out var classification))
            return;
